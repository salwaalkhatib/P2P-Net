...
class PMG(nn.Module):
    def __init__(self, model, feature_size, num_ftrs, classes_num, topn):
        ...
        self.pad_side = 224
        self.num_heads = 8
        self.attn_emb_dim = self.num_ftrs//4
        ...
        self.self_attn_1 = nn.MultiheadAttention(self.attn_emb_dim, self.num_heads)
        # Inverted bottleneck MLP
        self.reg_mlp1 = nn.Sequential(
            nn.Linear(self.attn_emb_dim * self.topn, self.attn_emb_dim * self.topn * 4),
            nn.ELU(inplace=True),
            nn.Linear(self.attn_emb_dim * self.topn * 4, self.num_ftrs//2)
        )
        self.self_attn_2 = nn.MultiheadAttention(self.attn_emb_dim, self.num_heads)
        # Inverted bottleneck MLP
        ...
        self.self_attn_3 = nn.MultiheadAttention(self.attn_emb_dim, self.num_heads)
        # Inverted bottleneck MLP
        ...
        # Projection layer from self.num_ftrs//2 to self.att_emb_dim
        self.projection = nn.Parameter(torch.empty(self.num_ftrs//2, self.attn_emb_dim))
        nn.init.normal_(self.projection, std=self.attn_emb_dim ** -0.5)
